#!/usr/bin/env python3
"""Render a generic UiDo course source skeleton for human geometry QA.

OSM remains the structural source of truth: select the documented target
hole routes, associate source features deterministically to those routes, and
preserve the original geometry. Fairways may be represented by multipolygon
relations or complete standalone golf=fairway ways; relation members are resolved
only as a display representation. No geometry is invented, smoothed, or moved.
...
def select_target_holes(elements):
    ...
    If the source contains duplicate hole refs, the target manifest still
    documents refs 1..18; for duplicates we select the lowest OSM way ID and
    retain excluded routes in the QA report. This is deterministic source QA.
    """
    candidates = {}
    excluded = []
...
def fairway_sources(all_elements, fairway_elements, target_holes):
    ...
    # Complete standalone OSM fairway ways are authoritative source features
    # when they are not members of a fairway relation. This preserves complete
    # source geometry without duplicating relation member ways.
    ...
    for element in fairway_elements:
        ...
        # prevents unrelated golf-facility geometry inside the acquisition
        # boundary from being rendered.
...
def main():
    ...
    fig.text(
        0.5,
        0.945,
        "Canonical Course Packet • OSM geometry only • fairway relations + standalone ways • no invented or smoothed geometry",
        ha="center",
        fontsize=9,
    )
    ...
    report = {
        "schema": "uido.course.wireframe-qa.v0.1",
